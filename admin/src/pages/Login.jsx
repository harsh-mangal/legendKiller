      ...
      <section className="grid place-items-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={submit} className="mt-8 space-y-5 rounded-none border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
            <Field label="Admin Email" required><div className="relative"><Mail size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" /><Input type="email" autoComplete="email" className="pl-10" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} placeholder="admin@legendbornnutrition.com" required /></div></Field>
