...
import { Archive, Download, Eye, Film, ImagePlus, Pencil, Plus, RefreshCw, Search, Star, Trash2 } from "lucide-react";
...
const tabs = [
  ["basic", "Basics"], ["commercial", "Pricing & stock"], ["content", "Product information"], ["media", "Infographics & video"], ["compliance", "Compliance"], ["publishing", "SEO & visibility"],
];
...
export default function Products() {
  ...
  const visible = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      return (!categoryFilter || String(product.category?._id || product.category) === categoryFilter)
        ...
        && (!keyword || [product.name, product.sku, product.slug, product.category?.name].some((value) => String(value || "").toLowerCase().includes(keyword)));
    });
  ...
  return <div className="space-y-6">
    ...
    <Modal open={editorOpen} onClose={() => setEditorOpen(false)} title={editingId ? "Edit product" : "Add product"} description="Fields marked with * are required. New gallery, infographic or video files replace only their corresponding current set." size="xl" footer={<div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="secondary" onClick={() => setEditorOpen(false)}>Cancel</Button><Button onClick={submit} disabled={saving}>{saving ? "Saving product…" : editingId ? "Update product" : "Create product"}</Button></div>}>
      <div className="mb-6 flex gap-2 overflow-x-auto border-b border-stone-200 pb-3">{tabs.map(([value, label]) => <button key={value} type="button" onClick={() => setActiveTab(value)} className={`whitespace-nowrap rounded-lg px-3 py-2 text-xs font-bold ${activeTab === value ? "bg-brand-800 text-white" : "text-stone-600 hover:bg-stone-100"}`}>{label}</button>)}</div>
